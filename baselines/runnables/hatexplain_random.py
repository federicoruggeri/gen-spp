...
def random_hl(
    ...
    if not len(valid_indexes):
        valid_indexes = np.arange(len(hl_true))

    hl_hat = np.zeros(len(hl_true))[valid_indexes]
    sampled_indexes = np.random.choice(np.arange(len(valid_indexes)), size=min(hl_size, len(valid_indexes)),
                                       replace=False)
...
if __name__ == '__main__':
    ...
    save_path = Path(__file__).parent.parent.resolve().joinpath('results', 'toy_sequence', 'baseline_no_highlight')
