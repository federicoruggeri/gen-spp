...
from components.model import HlModel
...
from configurations.model import FRConfig
from modeling.spp import FR
...
if __name__ == '__main__':
    ...
    save_path = Path(__file__).parent.parent.resolve().joinpath('results', 'hatexplain', f'fr_skew_{skew_epochs}')
    ...
    config = FRConfig.from_config(key=ConfigKey(dataset='hatexplain', tags={'spp', 'fr'}))
    ...
    for seed in config.seeds:
        ...
        model = FR(vocab_size=collator.vocab_size,
        ...
        if should_train:
            wandb_logger = WandbLogger(project='hlext-hatexplain', name=f'fr_skew_{skew_epochs}')
        else:
            wandb_logger = None
        ...
        model = HlModel.load_from_checkpoint(checkpoint_path=ckpt_filepath,
                                             model=model.model,
                                             val_classification_metrics=MetricCollection({
