...
from components.model import GRATHlModel
from components.processing import HighlightDataset, GloVeEmbedderCollator, HighlightExtractor
from configurations.base import ConfigKey
from configurations.model import GRATConfig
from modeling.spp import GRAT, GRATGuider
...
if __name__ == '__main__':
    ...
    parser.add_argument('--skew', '-s', default=10, type=int)
    args = parser.parse_args()
    skew_epochs = args.skew

    save_path = Path(__file__).parent.parent.resolve().joinpath('results', 'hatexplain', f'grat_skew_{skew_epochs}')
    ...
    config = GRATConfig.from_config(key=ConfigKey(dataset='hatexplain', tags={'spp', 'grat'}))
    ...
    collator = GloVeEmbedderCollator(name=config.embedding_name,
                                     dim=config.embedding_dim,
                                     use_pretrained_only=config.use_pretrained_only,
                                     cache_dir=data_dir.parent.joinpath('embeddings'))
    ...
    for seed in config.seeds:
        ...
        model = GRAT(vocab_size=collator.vocab_size,
        ...
        guider = GRATGuider(vocab_size=collator.vocab_size,
                            embedding_dim=config.embedding_dim,
                            hidden_size=config.hidden_size,
                            classification_head=config.classification_head,
                            dropout_rate=config.dropout_rate,
        ...
        model = GRATHlModel(model=model,
                            guider=guider,
                            num_classes=config.num_classes,
                            optimizer_class=config.optimizer_class,
                            optimizer_kwargs=config.optimizer_kwargs,
                            val_classification_metrics=MetricCollection({
                                'f1': F1Score(task='multiclass', average='macro', num_classes=2)}),
                            ...
                            guide_coefficient=config.guide_coefficient,
                            jsd_coefficient=config.jsd_coefficient,
        ...
        if should_train:
            wandb_logger = WandbLogger(project='hlext-hatexplain', name=f'grat_skew_{skew_epochs}')
        else:
            wandb_logger = None
        ...
        model = GRATHlModel.load_from_checkpoint(checkpoint_path=ckpt_filepath,
                                                 model=model.model,
                                                 guider=model.guider,
                                                 val_classification_metrics=MetricCollection({
